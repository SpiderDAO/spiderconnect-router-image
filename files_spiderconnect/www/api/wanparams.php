...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	...
	} else if ($_POST['wan-connection'] == "static") {
		$cli->execute("uci set network.wan.proto='static'");
		$cli->execute("uci set network.wan.ipaddr='".$_POST['static-ip']."'");
		$cli->execute("uci set network.wan.netmask='".$_POST['netmask']."'");
		$cli->execute("uci set network.wan.gateway='".$_POST['gateway']."'");
		$cli->execute("uci set network.wan.dns='".$_POST['dns']."'");
		$cli->execute("uci set network.wgclient.customdns='".$_POST['dns']."'");
	} else if ($_POST['wan-connection'] == "pppoe") {
		$cli->execute("uci set network.wan.proto='pppoe'");
		$cli->execute("uci set network.wan.username='".$_POST['username']."'");
		$cli->execute("uci set network.wan.password='".$_POST['password']."'");
