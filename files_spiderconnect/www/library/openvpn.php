...
class openvpn
{
    ...
    public function change_server($server)
    {
		...
		if (isset($wgprofile['message']))
		{
			...
			return json_encode(['name' => $this->get_server_detail()['name'],
			'waited' => $waited,
			'flag' => $this->get_server_detail()['name']], JSON_PRETTY_PRINT);
			
		} else if ($wgprofile !== false && !empty($wgprofile) && $wgprofile != "null") {
			...
			$status = $cli->execute("uci set network.$WG_IF.proto=\"wireguard\"");
			$status = $cli->execute("uci set network.$WG_IF.private_key=\"$WG_KEY\"");
			$status = $cli->execute("uci set network.$WG_IF.listen_port=\"$WG_PORT\"");
			$status = $cli->execute("uci add_list network.$WG_IF.addresses=\"$WG_ADDR\"");
			$status = $cli->execute("uci set network.$WG_IF.mtu=\"1300\"");
			$status = $cli->execute("uci commit network");
			 
			# Add VPN peers
			$killswitch   = trim($cli->execute("uci get network.wgclient.killswitch"));
			$dnsunblocker = trim($cli->execute("uci get network.wgclient.dnsunblocker"));
			$customdns    = trim($cli->execute("uci get network.wgclient.customdns"));
			
			$status = $cli->execute("uci -q delete network.wgclient");
			$status = $cli->execute("uci set network.wgclient=\"wireguard_$WG_IF\"");
			$status = $cli->execute("uci set network.wgclient.public_key=\"$WG_PUB\"");
			$status = $cli->execute("uci set network.wgclient.preshared_key=\"$WG_PSK\"");
			$status = $cli->execute("uci set network.wgclient.name=\"$server\"");
			$status = $cli->execute("uci set network.wgclient.endpoint_host=\"$WG_ENDPOINT_HOST\"");
			$status = $cli->execute("uci set network.wgclient.endpoint_port=\"$WG_ENDPOINT_PORT\"");
			$status = $cli->execute("uci add_list network.wgclient.allowed_ips=\"$WG_ALLOWED\"");
			$status = $cli->execute("uci set network.wgclient.persistent_keepalive=\"25\"");
			$status = $cli->execute("uci set network.wgclient.route_allowed_ips=\"1\"");
			$status = $cli->execute("uci set network.wgclient.killswitch=\"".$killswitch."\"");
			$status = $cli->execute("uci set network.wgclient.dnsunblocker=\"".$dnsunblocker."\"");
			if (trim($customdns) !== "") {
				$status = $cli->execute("uci set network.wgclient.customdns=\"".$customdns."\"");
			}
			$status = $cli->execute("uci commit network");
			...
			file_put_contents("/tmp/vpnstatus.flag", $this->get_server_detail()['name']);
			...
			file_put_contents("/tmp/error.flag", "Last error: Cant get VPN key for location.
<br/><br/>
...
Step 3) Factory reset your router by clicking <a href='/factoryreset.php' style='color: white;'>here</a>, and then reactivate your router.");
    ...
    public function get_status()
    {
        ...
        do {
            sleep(1);
            $seconds++;
        } while ((json_decode($cli->execute('ifstatus wg0')) == false
    ...
    {
        $this->command("uci set network.wg0.disabled=\"0\"; uci commit network;");
    }

    private function disable()
    {
        $this->command("uci set network.wg0.disabled=\"1\"; uci commit network;");
    }

    public function restart()
    {
        $this->command('(ubus call network.interface.wg0 down; ubus call network.interface.wan down; ubus call network.interface.wan up; ubus call network.interface.wg0 up;)  >/dev/null 2>&1 &' );
    }
