...
class spidervpn
{
    ...
    {
        $curl = new curl();
        $response = $curl->execute(SPIDERVPN_API_URL . 'auth/login', 'POST', ['email' => $email, 'password' => $password]);
    ...
    public function get_updates()
    {
        ...
        $curl = new curl();
        $response = $curl->execute(SPIDERVPN_API_URL . "get-router-updates", 'POST', ['version' => $version, 'key' => FASTESTVPN_API_KEY]);
    ...
    {	
        $curl = new curl();
        $response = $curl->execute(SPIDERVPN_API_URL . 'authentication/locations', 'GET', ['Content-Type' => "application/json", 'X-Requested-With' => "XMLHttpRequest"]);
		$response = json_decode($response);
		if ($response->error === 0)
