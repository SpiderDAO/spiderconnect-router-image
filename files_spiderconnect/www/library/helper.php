...
{
    if (
        strpos(file_get_contents("/etc/firewall.user"), "/cgi-bin/luci") ===
        false
    ) {
        return "lucion";
    } else {
...
function get_wifi_width5()
{
    ...
    if (!empty($GHZ5EXIST)) {
        ...
        if (
            $value == "HT40" &&
            exec("uci get wireless.radio$GHZ5.noscan") == 1
        ) {
            $value = "HT40";
        } elseif (
            $value == "VHT80" &&
            exec("uci get wireless.radio0.noscan") == 1
        ) {
            $value = "VHT80";
        } else {
            $value = "auto";
...
function get_wifi_width2()
{
    ...
    if (!empty($GHZ5EXIST)) {
        ...
        if (
            $value == "HT40" &&
            exec("uci get wireless.radio$GHZ2.noscan") == 1
        ) {
            //$value = "HT40";
        } elseif ($value == "HT20") {
            //$value = "HT20";
...
{
    $curl = new curl();
    $wgprofile = $curl->execute(
        SPIDERVPN_API_URL . 'authentication/tunnel-details',
        'POST',
        ['loc_name' => $location]
    );
    ...
    if (
        ...
        file_put_contents(
            "/www/config/" . $location . ".json",
            json_encode($wgprofile, JSON_PRETTY_PRINT)
        );
    } else {
        $curl2 = new curl();
        $wgprofile = $curl2->execute(
            SPIDERVPN_API_BACKUP_URL . 'authentication/tunnel-details',
            'POST',
            ['loc_name' => $location]
        );
        $wgprofile = json_decode($wgprofile, true);
        if (
            ...
            $wgprofile = json_decode(
                file_get_contents("/www/config/" . $location . ".json"),
                true
            );
        }
    }
...
function set_dns_servers($stop_wg0_first = false)
{
    ...
    if (
        get_dnsunblocker_status() == "wireguard" &&
        !empty(get_server_name()) &&
        !empty($wgprofile['Interface']['dns'])
    ) {
        ...
        foreach ($WG_DNS as $dns) {
            if (!empty(trim($dns['ip']))) {
                $status = exec(
                    "uci add_list network.wan.dns='" . $dns['ip'] . "'"
                );
                $status = exec(
                    "uci add_list network.wwan.dns='" . $dns['ip'] . "'"
                );
            }
        }
    } elseif (get_dnsunblocker_status() == "custom") {
        //custom dns servers
...
function check_activation()
{
    ...
    if (
        basename($_SERVER['PHP_SELF']) !== 'wanparams.php' &&
        basename($_SERVER['PHP_SELF']) !== 'wan-setup.php' &&
        basename($_SERVER['PHP_SELF']) !== 'activation.php' &&
...
function check_credentials()
{
    ...
    if ($vpnusername && $vpnpassword) {
        ...
        if (
            file_exists("/access_token.auth") &&
            filesize("/access_token.auth") > 0
        ) {
...
{
    if (stream_select($read = [$in], $write = null, $except = null, $tv_sec)) {
        return fgets($in);
    } else {
        return " ";
    }
